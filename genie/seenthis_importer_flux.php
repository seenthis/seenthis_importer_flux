...
function genie_seenthis_importer_flux($t) {
	...
	if (!defined('_IMPORT_FLUX_MULTIPLES')) {
		define('_IMPORT_FLUX_MULTIPLES', 1);
	}
...
function seenthis_importer_rss_article($article, $moi, $create = true) {
	...
	foreach ($urls as $i => $urlo) {
		...
		$urlo = preg_replace(
			...
			'\1spip.php?page=resume&id_article=\2',
			$urlo
		);
		...
		if (
			...
			INNER JOIN spip_me AS m ON t.uuid=m.uuid AND (t.tag = ' . sql_quote('http://' . $lien_flou) . ' OR t.tag = ' . sql_quote('https://' . $lien_flou) . ')';
	...
	sql_insertq('spip_me_share', ['id_me' => $id_me, 'id_auteur' => $moi['id_auteur'], 'date' => 'NOW()']);
	cache_message($id_me);
	return 1;
}
...
function seenthis_creer_message_local($article, $url) {
	...
	if (is_array($article['tags'])) {
		...
		foreach ($article['tags'] as $tag) {
			...
			} else { // les enclosures sont affichees sous forme de lien brut
				if (
					strstr(',enclosure,external,', ",$rel,")
		...
		foreach (extraire_balises($article['enclosures'], 'a') as $enc) {
			...
			if (
				...
				and false === strpos($message, $href)
			) {  # enclosure n'est pas dans le descriptif
				$tags[] = "\n" . $href;
			}
		}
	}
	...
	// s'il reste des &#x27; &#39; &#34; &#039; &#034; etc
	$message = preg_replace_callback('/(&#x?[0-9a-f]+;)/i', function ($m) {
